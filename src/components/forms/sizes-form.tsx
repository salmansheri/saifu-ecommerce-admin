...
import { SizesType, SizesValidation } from "@/lib/validations/sizes";
...
const SizesForm: React.FC<SizesFormProps> = ({ initialData, colors }) => {
  const params = useParams();
  const router = useRouter();
  const form = useForm<SizesType>({
    // @ts-ignore
    resolver: zodResolver(SizesValidation),
  ...
  const { mutate, isLoading } = useMutation({
    mutationFn: async ({ value, name }: SizesType) => {
      const payload = {
        value,
        name,
      ...
      if (initialData) {
        const { data } = await axios.patch(
          `/api/store/${params.storeId}/sizes/${initialData.id}`,
          payload
        );
        ...
        const { data } = await axios.post(
          `/api/store/${params.storeId}/sizes`,
          payload
        );
    ...
    onSuccess: (data) => {
      ...
      router.refresh();
      return toast({
        title: "Sizes  Successfully Created",
      });
    },
  });
      ...
      <CardHeader>
        <div>
          <CardTitle className="text-3xl">
            {initialData ? "Update Sizes" : " Create Sizes"}
          </CardTitle>
          <CardDescription>
            {initialData
              ? "Update Your Sizes"
              : "Create Your Sizes to Continue"}
          </CardDescription>
        </div>
        {initialData && (
