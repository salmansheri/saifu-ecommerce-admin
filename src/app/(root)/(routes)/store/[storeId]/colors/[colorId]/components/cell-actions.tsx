...
import { ColorColumsType } from "./column";
...
const CellActions: React.FC<CellActionsType> = ({ data }) => {
  ...
  const { mutate: onDelete } = useMutation({
    mutationFn: async () => {
      await axios.delete(`/api/store/${params.storeId}/colors/${data.id}`);
    },
    onError: (error) => {
      return toast({
        title: "Cannot Delete",
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem
          className="flex gap-x-2"
          onClick={() =>
            router.push(`/store/${params.storeId}/colors/${data.id}/update`)
          }
        >
